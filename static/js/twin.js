...
function setupEventListeners() {
    ...
    if (speedControl) {
        speedControl.addEventListener('input', function(e) {
            ...
            if (speedValue) speedValue.textContent = machineSpeed + '%';
        });
    }

    const productionModeSelect = document.getElementById('productionMode');
    if (productionModeSelect) {
        productionModeSelect.addEventListener('change', function(e) {
            productionMode = e.target.value;
        });
    }

    const startMachineBtn = document.getElementById('startMachine');
    const stopMachineBtn = document.getElementById('stopMachine');

    if (startMachineBtn) startMachineBtn.addEventListener('click', startMachine);
    if (stopMachineBtn) stopMachineBtn.addEventListener('click', stopMachine);

    // Simulation controls
    const runSimulationBtn = document.getElementById('runSimulation');
    const testScenarioBtn = document.getElementById('testScenario');
    ...
    if (testScenarioBtn) testScenarioBtn.addEventListener('click', testFailureScenario);
    if (optimizeLayoutBtn) optimizeLayoutBtn.addEventListener('click', optimizeLayout);
    ...
    if (fullscreenMode) fullscreenMode.addEventListener('click', toggleFullscreen);

    // Export and share
    const exportModel = document.getElementById('exportModel');
    const shareView = document.getElementById('shareView');

    if (exportModel) exportModel.addEventListener('click', exportModelFunc);
    if (shareView) shareView.addEventListener('click', shareViewFunc);
}
...
function updateSensorDisplay(data) {
    const tempElement = document.getElementById('twinTemp');
    const pressureElement = document.getElementById('twinPressure');
    const rpmElement = document.getElementById('twinRpm');
    const powerElement = document.getElementById('twinPower');

    if (tempElement) tempElement.textContent = `${Math.round(data.temperature)}°F`;
    if (pressureElement) pressureElement.textContent = `${data.pressure.toFixed(1)} bar`;
    if (rpmElement) rpmElement.textContent = `${Math.round(data.rpm)} rpm`;
    if (powerElement) powerElement.textContent = `${Math.round(data.power)} kW`;
